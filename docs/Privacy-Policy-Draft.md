...
and retention schedule are reviewed by qualified privacy counsel.
...
- Account identifiers supplied by the selected Google or Apple sign-in provider,
  including a provider subject identifier and, when authorized, email address.
...
- Security and reliability metadata such as correlation IDs, timestamps, response
  ...
  screenshots, credentials, tokens, or prompts.

Imported screenshots stay on the device and are temporary. Screenshot bytes and
local paths are not sent to the backend or AI provider. The user reviews the
normalized message sequence before any analysis or AI request can run.

## Why we process data

We process the minimum data necessary to provide an action requested by the user,
secure the service, enforce usage limits, administer subscriptions, support data
export and deletion, and meet legal obligations. The final legal basis for every
launch jurisdiction must be recorded in the approved data inventory before this
draft is published.

## Hosted AI processing

Only after a clear user action and disclosure, the minimum confirmed conversation
context needed for coaching may be sent through OpenRouter to the underlying
model provider selected for the user's server-verified plan. Current development
...
subprocessors, security terms, training setting, and retention/deletion behavior,
and those facts replace this paragraph where needed.

## Retention and deletion

Raw screenshots are not retained by the service. Confirmed conversations and
private reflections are retained until the user deletes them or the account,
subject to the final bounded retention schedule. Operational logs use a short,
documented retention period and contain no conversation content. Store entitlement
records retain pseudonymous transaction hashes only for subscription integrity and
legal accounting periods.

The app provides account export and deletion. Account deletion removes or
irreversibly de-identifies personal application data, subject only to legally
required records. Backup expiry and processor deletion timelines must be stated in
the final schedule.
...
applicable transfer mechanisms, representatives, regulator contacts, and request
handling timelines after counsel review.

## Changes and contact

Material changes will be disclosed before they take effect. Questions and privacy
